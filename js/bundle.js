...
function calc() {
  ...
  function calcTotal() {
    ...
    if (sex === 'female') {
      result.textContent = Math.round((447.6 + 9.2 * weight + 3.1 * height - 3.4 * age) * ratio);
    } else {
...
function slider({
  ...
  function buttonCurrent(btn) {
    if (slides.length < 10) {
      btn.textContent = `0${slideIndex}`;
    } else {
      btn.textContent = slideIndex;
    }
  }

  function setOpacityForDots(arr) {
...
window.addEventListener('DOMContentLoaded', () => {
  ...
  (0,_modules_timer__WEBPACK_IMPORTED_MODULE_3__.default)('.timer', '2021-03-21');
